...
#if 1
/*ov2710 demo 60fps raw data*/
static struct reg_list ov2710_demo_60fps_regs[] = {
...
//turn off AWB
    ...
    {0x3504, 0x04},
    {0x3405, 0x00},*/
//    {0x5001, 0x0},//AWB disable register
...
static struct reg_list ov2710_720P_regs[] = {
   ...
   {0x401c, 0x08},
    #if 0
 //  {0x3012, 0x01},
   {0x300f, 0x88}, //0x300f[7:6]: if  0 use 1
   {0x3011, 12}, //PLL = 40±¶,
 //  {0x3010, 0x10}, //PCLK config.80Mhz,24*40/1.5/2/1/4
   ...
   {0x300f, 0x88}, //0x300f[7:6]: if  0 use 1
   {0x3011, 0x28}, //PLL = 40±¶,
   {0x3010, 0x20}, //PCLK config.80Mhz,24*40/1.5/2/1/4   
...
//    ov2710_write_array(ov2710_demo_60fps_regs, ARRAY_SIZE(ov2710_demo_60fps_regs));

/*    Uint8 data;
    ov2710_read(0x3500, &data);
    DEV_DBG("0x%x", data);

    ov2710_read(0x3501, &data);
    DEV_DBG("0x%x", data);

    ov2710_read(0x3502, &data);
    DEV_DBG("0x%x", data);*/
