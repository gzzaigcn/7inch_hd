...
static struct reg_list ov5640_default_regs[] = {
  ...
  {0x3103,0x03},//
  {0x3017,0x00},//disable oe/disable io
  {0x3018,0x00},//
...
//  {0x302e,0x00},//
  ...
  {0x4300,0x00},//uyvy 0x30 0x00 raw 
  {0x501f,0x03},//by gzz {0x501f,0x00} 0x03:Raw DPC, 0x05:Raw CPI, 0x02 ISP dither
  ...
  /*HD=1,VD=0 : 0x20; HD=1,VD=1: 0x21; 
    HD= 0, VD = 0:0x22, HD=0, VD = 1:0x23*/  
  {0x4740,0x20},//hsync,vsync,clock pol,reference to application note,spec is wrong
...
static struct reg_list ov5640_720P_regs[] = //  //power down
{
...
//  //pll and clock setting
  ...
  {0x3503,0x00},  //AEC/AGC0 auto enable
  {0x350c,0x00},
  {0x350d,0x00},
...
//  {0x3503,0x00},//AEC enable
 ...
 {0x3500, 0x8},
  ...
  {0x302c,0xc0},//bit[7:6]: output drive capability
            //00: 1x   01: 2x  10: 3x  11: 4x 
...
static void ov5640_read(Uint16 reg_addr, Uint8 *value)
{
    ...
    ret = EVMDM6437_I2C_write(OV5640_I2C_ADDR, data, 2); //write reg addr
    if(ret < 0)
    {
        DEV_ERR("failed to write ov5640");
        return;
    }

    ret = EVMDM6437_I2C_read(OV5640_I2C_ADDR, value, 1); //read 1Byte data
    if(ret < 0)
    {
        DEV_ERR("failed to read ov5640");
        return;
    }
...
static void ov5640_write(Uint16 reg_addr, Uint8 value)
{
    ...
    ret = EVMDM6437_I2C_write(OV5640_I2C_ADDR, data, 3);
    if(ret < 0)
    {
        DEV_ERR("failed to write ov5640");
        return;
...
//    EVMDM6437_GPIO_setDir(GPIO63, GPIO_OUT);
//    EVMDM6437_GPIO_setOutput(GPIO63, 1);
    ...
    DEV_DBG("succeed to detect ov5640 on board£¡");
    ...
    ov5640_write_array(ov5640_io_enable_regs, ARRAY_SIZE(ov5640_io_enable_regs));//enable DVP I/O
